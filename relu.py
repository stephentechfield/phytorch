...
class relu:
    ...
    def backwardnest(self, D, lr, mu,l1,l2):
        ...
        self.gradw=lr*(self.A.T@(self.D*DerivRelu(self.Z))+l1*np.sign(self.weights)+l2*(self.weights))
        self.weights=self.weights-mu*self.delw
        self.gradb=lr*(np.sum(self.D*DerivRelu(self.Z),axis=0)+l1*np.sign(self.biases)+l2*(self.biases))
    ...
    def backwardada(self,D,lr,mu,nest,l1,l2):
        self.D=D
        self.gw=self.gw+(self.A.T@(self.D*DerivRelu(self.Z))+l1*np.sign(self.weights)+l2*(self.weights))**2
        self.gb=self.gb+(np.sum(self.D*DerivRelu(self.Z),axis=0)+l1*np.sign(self.biases)+l2*(self.biases))**2
        self.etaw=lr/(np.sqrt(self.gw+.000000001))
        self.etab=lr/(np.sqrt(self.gb+.000000001))
        self.Dout=(self.D*DerivRelu(self.Z))@self.weights.T
        if not nest:
            self.mw=mu*self.mw-self.etaw*(self.A.T@(self.D*DerivRelu(self.Z))+l1*np.sign(self.weights)+l2*(self.weights))
            self.mb=mu*self.mb-self.etab*(np.sum(self.D*DerivRelu(self.Z),axis=0)+l1*np.sign(self.biases)+l2*(self.biases))
    ...
    def backwardadanest(self, D, lr, mu,l1,l2):
        self.D=D
        ### remembering where we went and taking a step back
        self.gradw=self.etaw*(self.A.T@(self.D*DerivRelu(self.Z))+l1*np.sign(self.weights)+l2*(self.weights))
        self.weights=self.weights-mu*self.delw
        self.gradb=self.etab*(np.sum(self.D*DerivRelu(self.Z),axis=0)+l1*np.sign(self.biases)+l2*(self.biases))
        self.biases=self.biases-mu*self.delb
        ## moving forward with foresight
        self.delw=mu*self.delw-self.gradw
        self.weights+=self.delw
    ...
    def backwardrms(self,D,lr,gamma,mu,nest,l1,l2):
        self.D=D
        self.gw=(gamma*self.gw)+np.multiply((1-gamma),((self.A.T@(self.D*DerivRelu(self.Z)))+l1*np.sign(self.weights)+l2*(self.weights))**2)
        self.gb=(gamma*self.gb)+np.multiply((1-gamma),(np.sum(self.D*DerivRelu(self.Z),axis=0)+l1*np.sign(self.biases)+l2*(self.biases))**2)
        self.etaw=lr/(np.sqrt(self.gw+.000000001))
        self.etab=lr/(np.sqrt(self.gb+.000000001))
        self.Dout=(self.D*DerivRelu(self.Z))@self.weights.T
        if not nest:
    ...
    def backwardadam(self,D,lr,gamma,mu,count,l1,l2):
        self.D=D
        self.mw=mu*self.mw+(1-mu)*(self.A.T@(self.D*DerivRelu(self.Z))+l1*np.sign(self.weights)+l2*(self.weights))
        self.mb=mu*self.mb+(1-mu)*((np.sum(self.D*DerivRelu(self.Z),axis=0))+l1*np.sign(self.biases)+l2*(self.biases))
        self.gw=(gamma*self.gw)+np.multiply((1-gamma),(self.A.T@(self.D*DerivRelu(self.Z))+l1*np.sign(self.weights)+l2*(self.weights))**2)
        self.gb=(gamma*self.gb)+np.multiply((1-gamma),(np.sum(self.D*DerivRelu(self.Z),axis=0)+l1*np.sign(self.biases)+l2*(self.biases))**2)
